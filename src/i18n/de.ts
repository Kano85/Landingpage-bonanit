export const de = {
  // Header
  'nav.home': 'Startseite',
  'nav.about': 'Über mich',
  'nav.services': 'Meine Dienstleistungen',

  // Hero
  'hero.title': 'Besser schlafen mit liebevoller Bindung',
  'hero.subtitle':
    'Ich bin Laura Montoy, Gründerin von Bonanit, und ich begleite dich dabei, den Schlaf deines Babys zu verbessern – liebevoll und achtsam.',
  'hero.cta': 'Kennenlernen',

  // Questions
  'questions.q1': 'Sind Sie erschöpft und am Ende Ihrer Geduld?',
  'questions.q2':
    'Können Sie nachts nicht ausruhen und es beeinträchtigt bereits Ihre Gesundheit?',
  'questions.q3': 'Fühlen Sie sich von der Elternschaft überfordert?',
  'questions.q4': 'Fühlen Sie sich allein, mit oder ohne Unterstützungsnetz?',

  // Not Alone
  'alone.title': 'Sie müssen das nicht allein durchmachen',
  'alone.p1':
    'Man sagt Ihnen, dass es nur eine Phase ist, die schnell vorübergeht... aber das macht es nicht weniger schwierig. Schlafmangel kann Frustration verursachen und dazu führen, dass Sie diese besondere Phase nicht vollständig genießen. Und wenn Sie Ihre Familie auch noch weit weg von Ihrem Unterstützungsnetz aufziehen, wird alles noch schwieriger.',
  'alone.p2':
    'Mir ging es auch so. Ich habe die Mutterschaft in einem anderen Land entdeckt, weit weg von meiner Familie und meiner Sprache. Deshalb begleite ich heute andere Familien, damit sie das nicht allein durchmachen müssen.',

  // Pullquote
  quote:
    'Ich nehme Sie an die Hand, um die richtigen Veränderungen vorzunehmen und Ihre Nächte zu transformieren.',

  // Real Support
  'support.title': 'Hier haben Sie echte Unterstützung',
  'support.p1':
    'Wenn Sie spüren, dass Sie echte Begleitung brauchen, um den Schlaf Ihres Babys zu verbessern, werden Sie mit meinem 4-Wochen-Plan jemanden an Ihrer Seite haben.',
  'support.p2':
    'Ich werde einen respektvollen und personalisierten Schlafplan erstellen, und wir werden in Kontakt bleiben durch Videoanrufe und Nachrichten.',
  'support.p3':
    'Sie sind nicht allein: Besserer Schlaf ist möglich, ohne auf "diese Phase" zu warten.',

  // FAQ
  'faq.title': 'Häufig gestellte Fragen',
  'faq.q1': 'Wird mein Baby weinen?',
  'faq.a1':
    'Sehr wahrscheinlich nicht. Ich werde alles Mögliche tun, um das zu vermeiden, auch wenn es bedeutet, langsamer vorzugehen. Mein Plan für Kinderschlaf passt sich an, wenn es Tränen gibt, und ist zutiefst respektvoll. Wir folgen immer dem Rhythmus Ihres Babys und priorisieren sein Wohlbefinden.',
  'faq.q2': 'Ist der Plan mit Stillen und Co-Sleeping kompatibel?',
  'faq.a2':
    'Ja, vollständig. Der Schlafplan passt sich Ihren Wünschen und Bedürfnissen an. Ich befürworte bindungsorientierte Erziehung und respektiere das Stillen und Co-Sleeping. Das Wesentliche ist, gemeinsam den besten Weg zu finden, damit Ihr Baby gut schläft und sich sicher fühlt.',
  'faq.q3': 'Muss ich zuhause bleiben und Reisen vermeiden?',
  'faq.a3':
    'Während der vier Wochen des Schlafplans empfehle ich, die Routine vorrangig zu behandeln und den Schlafplatz Ihres Babys nicht zu verändern. Wir führen wichtige Veränderungen in die Schlafenszeit-Gewohnheiten Ihres Babys ein, und es ist nicht ratsam, gleichzeitig andere große Veränderungen hinzuzufügen. Ruhe und Routine werden in diesem Prozess der Schlüssel sein.',
  'faq.q4': 'Muss ich für immer strikte Zeitpläne einhalten?',
  'faq.a4':
    'Während des Plans ist es wichtig, die Zeitpläne so genau wie möglich einzuhalten, um Überermüdung zu vermeiden, die die Anpassung an neue Gewohnheiten erschwert. Sobald Ihr Baby diese erworben hat, können Sie flexibler sein und Ausnahmen machen. Tatsächlich passen sich Kinder mit guten Schlafgewohnheiten besser an Zeitplanänderungen an.',
  'faq.q5': 'Wie lange dauert es, bis Ergebnisse sichtbar sind?',
  'faq.a5':
    'Das hängt von jedem Baby ab, aber im Allgemeinen bemerkt man die ersten Ergebnisse nach etwa zwei Wochen und es wird sich weiterhin während einiger Wochen verbessern. Da der Plan respektvoll ist, folgen wir immer dem Rhythmus Ihres Babys, daher können die Fortschritte manchmal länger dauern. Das Wichtigste ist, konsistent zu sein und dem Prozess zu vertrauen, um seinen Schlaf zu verbessern.',
  'faq.q6':
    'Was passiert, wenn etwas nicht funktioniert oder Sie sich nicht wohlfühlen?',
  'faq.a6':
    'Das Wichtigste in diesem Prozess ist die Kommunikation mit mir. Wenn sich etwas nicht richtig anfühlt oder nicht funktioniert, ist es nicht das Richtige. Ich brauche Ihre Offenheit und möchte wissen, wie Sie sich fühlen, um den Plan bei Bedarf anzupassen. Zusammen finden wir den Weg, der am besten zu Ihrer Familie und zum Schlaf Ihres Babys passt.',

  // Testimonials
  'testimonials.title': 'So haben einige Familien die Erfahrung mit mir gemacht…',

  // CTA Final
  'cta.final': 'Sollen wir einen kostenlosen Videoanruf vereinbaren?',

  // Footer
  'footer.tagline':
    'Ich begleite Familien dabei, den Schlaf ihrer Babys mit einem respektvollen, persönlichen und personalisierten Ansatz zu verbessern, um die Mutterschaft mit mehr Ruhe und Vertrauen zu leben.',
  'footer.follow': 'Folge mir',
  'footer.info': 'Information',
  'footer.nav': 'Navigation',
  'footer.connect': 'Verbinden Sie sich mit mir',
  'footer.connect_desc':
    'Lassen Sie uns ein Gespräch über die Verbesserung des Schlafes Ihres Babys führen.',
  'footer.schedule': 'Anruf buchen',

  // About Page (Über mich)
  'about.hero.title': 'Ich bin Laura Montoy',
  'about.hero.p1':
    'Vor mehr als einem Jahrzehnt bin ich nach Berlin gezogen, wo meine Familie geboren wurde und eine neue Version von mir.',
  'about.hero.p2':
    'Von Ausbildung bin ich Biotechnologe und Oberstufenlehrerin. Ich habe die letzten acht Jahre Biologie und Spanisch unterrichtet und meine Liebe zur Wissenschaft war immer in meiner Art, die Welt zu sehen und zu verstehen präsent, aber es war die Mutterschaft, die mich wirklich transformierte.',
  'about.how.title': 'Wie alles begann',
  'about.how.p1':
    'Als mein erstes Kind geboren wurde, entdeckte ich etwas Unerwartetes: eine absolute Faszination für Kinderschlaf. Ich las, forschte und bildete mich weiter, um es nachts besser zu begleiten und uns beiden zu helfen, besser zu schlafen.',
  'about.how.p2':
    'Mit der Ankunft meiner zweiten Tochter ergaben sich neue Herausforderungen. Ich beschloss, mich nach zwei Monaten als Kinderschlafratsgeberin zu zertifizieren. Es war ein intensiver Prozess aus Studium, Praxis und Engagement, auf den ich sehr stolz bin.',
  'about.how.p3':
    'Ich habe viele Erfahrungen gemacht: natürliche Geburt und Kaiserschnitt, ausschließlich Stillen und Formel, ein verschlafenes Baby und eines, das nicht so sehr schläft. All das lehrte mich, dass es nicht nur eine Art zu erziehen oder zu schlafen gibt.',
  'about.work.title': 'Wie ich arbeite',
  'about.work.p1':
    'Heute kombiniere ich meine zwei Leidenschaften: Ich unterrichte weiterhin Teilzeit und widme den Rest meiner Energie der Begleitung von Familien als Schlafberaterin.',
  'about.work.p2':
    'Ich arbeite mit einem respektvollen Ansatz, der auf Wissenschaft basiert und auf jede Familie abgestimmt ist. Mit oder ohne Co-Sleeping, Stillen oder Flasche, im gleichen Zimmer oder in separaten Betten: alles ist gültig. Ich schaffe echte und nachhaltige Lösungen.',
  'about.work.p3':
    'Sie zu begleiten ist nicht nur ein Job, es ist meine Berufung. Ich bin hier, um Ihnen zu helfen, den Schlaf wieder Teil Ihres Lebens zu machen, denn gut schlafen sollte kein Luxus sein.',
  'about.cta.title': 'Buchen Sie Ihre kostenloses Termin',
  'about.cta.p':
    'Lassen Sie uns über Ihre Bedürfnisse sprechen und wie ich Sie auf dem Weg zu besserem Schlaf begleiten kann',
  'about.cta.button': 'Lassen Sie uns kennenlernen!',

  // Services Page (Meine Dienstleistungen)
  'services.hero.title': 'Meine Dienstleistungen',
  'services.hero.subtitle':
    'Wählen Sie den Plan, der am besten zu Ihrer Situation passt',
  'services.discount.title': '10% Rabatt',
  'services.discount.p1':
    'Wenn Sie bereits mit mir gearbeitet haben, erhalten Sie 10% Rabatt auf alle zukünftigen Dienstleistungen!',
  'services.discount.p2':
    'Ich schätze Ihr Vertrauen und möchte Sie in jeder neuen Phase mit dem gleichen Engagement und der gleichen Nähe begleiten.',
  'services.cta.title': 'Buchen Sie Ihren kostenlosen Termin',
  'services.cta.p':
    'Lassen Sie uns über Ihre Bedürfnisse sprechen und wie ich Sie auf dem Weg zu besserem Schlaf begleiten kann',
  'services.cta.button': 'Lassen Sie uns kennenlernen!',
  'services.faq.title': 'Häufig gestellte Fragen',
  'services.faq.q1': 'Wie alt sind die Babys?',
  'services.faq.a1':
    'Ich arbeite mit Familien, die Babys von Neugeborenen bis 4 Jahren haben. Jedes Alter hat seine Besonderheiten und ich passe den Plan an die spezifischen Bedürfnisse Ihres Babys an.',
  'services.faq.q2': 'Was ist, wenn ich keine Ergebnisse sehe?',
  'services.faq.a2':
    'Schlafveränderungen brauchen Zeit und Geduld. Während unserer Zusammenarbeit werden wir Anpassungen vornehmen, um sicherzustellen, dass der Plan für Ihre Familie funktioniert. Mein Ziel ist, dass Sie echte Unterstützung im Prozess spüren.',
  'services.faq.q3': 'Kann ich meinen Plan während der Begleitung ändern?',
  'services.faq.a3':
    'Natürlich. Wenn Sie jederzeit mehr oder weniger Unterstützung brauchen, können wir den Plan nach Ihren Bedürfnissen anpassen. Ihr Komfort und Vertrauen ist das Wichtigste.',
  'services.faq.q4': 'Was genau ist in der WhatsApp-Verfolgung enthalten?',
  'services.faq.a4':
    'Ich überprüfe Ihr tägliches Schlafprotokoll, beantworte Ihre Fragen, gibt Ihnen Orientierung über Änderungen im Plan, wenn nötig, und achte auf Ihre Bedenken. Es ist kein kontinuierlicher Chat, sondern strukturierte Begleitung in Ihrem Tempo.',

  // Service Plans (Dienste)
  'service.plan1.title': 'Schlafplan mit Begleitung',
  'service.plan1.duration': '4 Wochen',
  'service.plan1.desc1':
    'Dieser Plan ist für Sie, wenn Sie spüren, dass Sie echte Unterstützung auf dem Weg zum Kinderschlaf benötigen. Vier Wochen lang werde ich an Ihrer Seite sein, Fragen beantworten, Unsicherheiten beruhigen und den Schlafplan anpassen, je nachdem was Ihr Baby... und Sie brauchen.',
  'service.plan1.desc2':
    'Jeden Tag überprüfe ich die Schlafregister und achte auf jeden kleinen Fortschritt oder jede Schwierigkeit, denn manchmal brauchen wir nur zu wissen, dass wir nicht allein sind.',
  'service.plan1.f1':
    'Schlafanalyse durch Anfangsfragebogen und Schlafprotokoll',
  'service.plan1.f2': 'Erstellung eines spezifischen Schlafplans für Ihr Baby',
  'service.plan1.f3': '3 Videoanrufe à 40 Min',
  'service.plan1.f4': 'Tägliche Begleitung per WhatsApp',
  'service.plan1.f5': 'Schriftliche Abschlussempfehlungen',
  'service.plan1.button': 'Verfügbarkeit anfordern',

  'service.plan2.title': 'Planverlängerung',
  'service.plan2.duration': '4 weitere Wochen',
  'service.plan2.desc1':
    'Wenn Sie nach Abschluss Ihres Schlafplans spüren, dass noch mehr Arbeit zu tun ist, können Sie die Begleitung um vier weitere Wochen verlängern, mit einem speziellen Rabatt für die vorherige Zusammenarbeit.',
  'service.plan2.desc2':
    'Wir werden den Schlaf Ihres Babys weiter verfeinern mit täglicher Begleitung und zwei zusätzlichen Videoanrufen, ausreichend um sicher und selbstbewusst voranzukommen. Dies ist eine Option für diejenigen, die weitere Zeit lang Unterstützung möchten.',
  'service.plan2.f1': 'Anpassungen und Verfeinerung des Schlafplans',
  'service.plan2.f2': '2 Videoanrufe à 40 Min',
  'service.plan2.f3': 'Tägliche Begleitung per WhatsApp',
  'service.plan2.f4': 'Schriftliche Abschlussempfehlungen',
  'service.plan2.button': 'Verfügbarkeit anfordern',

  'service.plan3.title': 'Einzelsitzung',
  'service.plan3.duration': '45 Minuten',
  'service.plan3.desc1':
    'Diese Sitzung ist für Familien gedacht, die bereits an dem Schlaf ihres Babys gearbeitet haben – mit mir oder auf eigene Faust – und auf eine konkrete Schwierigkeit stoßen: eine Regression, eine Routineänderung oder eine neue Phase.',
  'service.plan3.desc2':
    'Während des Videoanrufs werden wir die Situation gemeinsam analysieren und ich gebe Ihnen einen klaren und praktischen Leitfaden, um das Gleichgewicht sicher wiederzugewinnen.',
  'service.plan3.f1': 'Vorheriger Fragebogen',
  'service.plan3.f2': 'Videoanruf à 45 Min',
  'service.plan3.f3': 'Schriftliche Abschlussempfehlungen',
  'service.plan3.button': 'Sitzung buchen',

  // Footer Links
  'links.legal.legal': 'Impressum',
  'links.legal.terms': 'Allgemeine Geschäftsbedingungen',
  'links.legal.privacy': 'Datenschutzerklärung',
  'links.nav.about': 'Über mich',
  'links.nav.services': 'Meine Dienstleistungen',
  'links.nav.contact': 'Kontakt',

  // Legal Pages
  'legal.title': 'Impressum',
  'legal.subtitle':
    'Rechtliche Informationen und Angaben zum Diensteanbieter von Bonanit.',
  'legal.section': 'Rechtliche Informationen',
  'legal.provider.title': 'Diensteanbieter',
  'legal.contact.title': 'Kontaktmöglichkeiten',
  'legal.contact.emailLabel': 'E-Mail-Adresse:',
  'legal.contact.email': 'info@bonanitsuenoinfantil.com',
  'legal.contact.other': 'Weitere Kontaktmöglichkeiten: Instagram',
  'legal.company.title': 'Angaben zum Unternehmen',
  'legal.disputes.title': 'Verbraucherstreitbeilegung',
  'legal.disputes.text':
    'Wir sind nicht bereit und nicht verpflichtet, an Streitbeilegungsverfahren vor einer Verbraucherschlichtungsstelle teilzunehmen.',
  'legal.social.title': 'Soziale Medien und weitere Onlinepräsenzen',
  'legal.social.text':
    'Dieses Impressum gilt auch für die folgenden Social-Media-Präsenzen und Onlineprofile:',
  'legal.back': 'Zurück zur Startseite',
  'legal.data.title': 'Vorbehalt der Nutzung für Text und Data Mining',
  'legal.data.text':
    'Jede Verwendung der Inhalte dieser Website für automatisierte Analyse-, Extraktions- oder Data-Mining-Prozesse bleibt vorbehalten, sofern keine ausdrückliche Genehmigung vorliegt.',
  'legal.disclaimer.title': 'Hinweise zu Haftung und Schutzrechten',
  'legal.disclaimer.exemption.title': 'Haftungsausschluss',
  'legal.disclaimer.exemption.text':
    'Die Inhalte dieses Onlineangebots wurden sorgfältig und nach unserem aktuellen Kenntnisstand erstellt, dienen jedoch nur der Information und entfalten keine rechtlich bindende Wirkung, sofern es sich nicht um gesetzlich verpflichtende Informationen handelt. Wir behalten uns vor, Inhalte ganz oder teilweise zu ändern oder zu löschen, soweit vertragliche Verpflichtungen unberührt bleiben. Alle Angebote sind freibleibend und unverbindlich.',
  'legal.disclaimer.copyright.title': 'Urheberrechte und Markenrechte',
  'legal.disclaimer.copyright.text':
    'Alle auf dieser Website dargestellten Inhalte wie Texte, Fotografien, Grafiken, Marken und Handelsnamen sind durch die jeweiligen Schutzrechte geschützt. Ihre Nutzung, Vervielfältigung oder sonstige Verwertung unterliegt unseren Rechten oder den Rechten der jeweiligen Urheber und Rechteinhaber.',
  'legal.disclaimer.legal.title': 'Hinweise auf Rechtsverstöße',
  'legal.disclaimer.legal.text':
    'Sollten Sie innerhalb unseres Internetauftritts Rechtsverstöße bemerken, bitten wir um einen entsprechenden Hinweis. Rechtswidrige Inhalte und Links werden wir nach Kenntnisnahme unverzüglich entfernen.',
  'legal.images.title': 'Bildnachweise',
  'legal.images.text': 'Bildquellen und Urheberrechtshinweise:',
  'legal.images.sources':
    'Freepik (www.freepik.com) oder von uns selbst erstellt.',
  'legal.images.generator':
    'Erstellt mit dem kostenlosen Datenschutz-Generator.de von Dr. Thomas Schwenke',
  'legal.contact.sidebarTitle': 'Kontakt',
  'legal.contact.text':
    'Bei Fragen zu diesem Impressum können Sie an folgende Adresse schreiben',

  // Terms and Conditions
  'terms.title': 'Allgemeine Geschäftsbedingungen',
  'terms.subtitle':
    'Allgemeine Vertragsbedingungen für die Schlafberatungsleistungen von Bonanit im Bereich Babyschlaf.',
  'terms.section1.title': '1. Geltungsbereich',
  'terms.section1.p1':
    'Diese Allgemeinen Geschäftsbedingungen (AGB) gelten für alle Leistungen der Babyschlafberatung, die von Bonanit / Laura Montoy Blanca gegenüber Privatkundinnen und Privatkunden im Sinne von §13 BGB erbracht werden.',
  'terms.section1.p2':
    'Die AGB sind Bestandteil aller Verträge zwischen der Beraterin und der Kundin bzw. dem Kunden.',
  'terms.section2.title': '2. Gegenstand der Leistung',
  'terms.section2.p1':
    'Die Beraterin bietet eine individuelle Begleitung zur Verbesserung des Schlafs von Babys und Kleinkindern an. Ziel ist es, passende und realistische Lösungen zu finden, die die Werte, Bedürfnisse und das Tempo jeder Familie respektieren.',
  'terms.section2.p2':
    'Die Leistung ersetzt keine medizinische oder psychologische Beratung.',
  'terms.section3.title': '3. Leistungserbringung',
  'terms.section3.item1':
    '3.1. Die Beraterin begleitet die Familie 28 Kalendertage lang zwischen 9:00 und 21:00 Uhr und bietet individuelle Empfehlungen per Videotelefonie, Nachrichten und Auswertung von Schlafprotokollen an.',
  'terms.section3.item2':
    '3.2. Die Beraterin handelt professionell, empathisch und vertraulich und passt ihre Empfehlungen an den Verlauf des Prozesses an.',
  'terms.section3.item3':
    '3.3. Werden die gewünschten Fortschritte innerhalb des vereinbarten Zeitraums nicht erreicht, kann die Beraterin die Begleitung einmalig und ohne zusätzliche Kosten um bis zu fünf Kalendertage verlängern. Die genaue Dauer innerhalb dieser Grenze bestimmt die Beraterin.',
  'terms.section3.item4':
    '3.4. Nach Ablauf dieser Verlängerungstage kann, wenn eine der Parteien weiterhin Verbesserungspotenzial sieht, eine Verlängerung des Plans zu einem gesondert vereinbarten geringen Preis angeboten werden.',
  'terms.section3.item5':
    '3.5. Die Beraterin behält sich das Recht vor, die Begleitung zu pausieren, wenn Anzeichen für ein mögliches medizinisches Problem festgestellt werden, das vor einer Fortsetzung professionell abgeklärt werden sollte.',
  'terms.section4.title': '4. Pflichten der Kundin bzw. des Kunden',
  'terms.section4.item1':
    '4.1. Die Kundin bzw. der Kunde verpflichtet sich zu einer offenen, ehrlichen und regelmäßigen Kommunikation, damit die Empfehlungen passend angepasst werden können.',
  'terms.section4.item2':
    '4.2. Die Empfehlungen der Beraterin sind weder verpflichtend noch starr. Wenn sich etwas für die Familie nicht richtig anfühlt, soll dies mitgeteilt werden, damit Alternativen gefunden werden können.',
  'terms.section4.item3':
    '4.3. Die Kundin bzw. der Kunde verpflichtet sich, die vereinbarten Empfehlungen im Rahmen der eigenen Möglichkeiten umzusetzen und über relevante Veränderungen in Gesundheit oder Verhalten des Babys zu informieren.',
  'terms.section4.item4':
    '4.4. Die Kundin bzw. der Kunde bestätigt, dass beim Baby oder Kind keine Symptome vorliegen, die eine dringende ärztliche Beurteilung erfordern. Bei Unsicherheiten über die Gesundheit des Kindes sind medizinische Fachpersonen zu konsultieren.',
  'terms.section4.item5':
    '4.5. Die Kundin bzw. der Kunde verpflichtet sich, keine Dokumente, individuellen Pläne, Empfehlungen oder sonstigen von der Beraterin erstellten Materialien ohne ausdrückliche Zustimmung weiterzugeben, zu vervielfältigen oder zu verbreiten.',
  'terms.section5.title': '5. Charakter der Begleitung',
  'terms.section5.p1':
    '5.1. Die von der Beraterin angebotene Begleitung basiert auf individuellen Empfehlungen und auf der Beobachtung der Entwicklung jeder Familie. Daher kann kein konkretes Ergebnis innerhalb eines bestimmten Zeitraums garantiert werden.',
  'terms.section5.p2':
    '5.2. Der Verlauf des Prozesses hängt unter anderem von der konsequenten Umsetzung der Empfehlungen, von den Bedürfnissen des Babys und von den besonderen Umständen jeder Familie ab.',
  'terms.section6.title': '6. Vertraulichkeit und Urheberrechte',
  'terms.section6.item1':
    '6.1. Alle geteilten personenbezogenen Informationen werden streng vertraulich behandelt und ohne Einwilligung nicht an Dritte weitergegeben.',
  'terms.section6.item2':
    '6.2. Sämtliche überlassenen Materialien (Dokumente, Empfehlungen, individuelle Pläne) sind ausschließlich für den Gebrauch der Familie bestimmt. Eine Vervielfältigung, Verbreitung oder Weitergabe an Dritte ist untersagt. Die Inhalte sind urheberrechtlich geschützt und bleiben Eigentum der Beraterin.',
  'terms.section7.title': '7. Datenschutz',
  'terms.section7.item1':
    '7.1. Die Beraterin beachtet die Vorgaben der Datenschutz-Grundverordnung (DSGVO). Personenbezogene Daten werden ausschließlich zur Erbringung der vereinbarten Leistung verwendet.',
  'terms.section7.item2':
    '7.2. Die Kundin bzw. der Kunde hat das Recht auf Auskunft, Berichtigung, Löschung und Einschränkung der Verarbeitung gemäß Art. 15 bis 21 DSGVO.',
  'terms.section7.item3':
    '7.3. Die Daten werden nur so lange gespeichert, wie dies für die Begleitung und für gesetzliche Aufbewahrungspflichten erforderlich ist.',
  'terms.section8.title': '8. Haftung',
  'terms.section8.item1':
    '8.1. Die Beraterin haftet nicht für Folgen, die aus einer Nichtanwendung, nur teilweisen Anwendung oder eigenmächtigen Änderung der Empfehlungen entstehen.',
  'terms.section8.item2':
    '8.2. Die Beratung ist keine medizinische Leistung. Die Beraterin übernimmt keine Haftung für Fehlinterpretationen, nicht getroffene medizinische Entscheidungen oder nicht mitgeteilte gesundheitliche Probleme.',
  'terms.section8.item3':
    '8.3. Die Haftung der Beraterin für Schäden ist auf Vorsatz und grobe Fahrlässigkeit gemäß §276 BGB beschränkt. Ansprüche wegen mittelbarer Schäden oder wirtschaftlicher Verluste sind ausgeschlossen.',
  'terms.section9.title': '9. Preise und Zahlung',
  'terms.section9.item1':
    '9.1. Der Preis der Begleitung richtet sich nach dem gebuchten Paket.',
  'terms.section9.item2':
    '9.2. Die Zahlung ist vor Beginn der Leistung zu leisten.',
  'terms.section9.item3':
    '9.3. Eine Ratenzahlung ist nur nach ausdrücklicher vorheriger Vereinbarung mit der Beraterin möglich.',
  'terms.section10.title': '10. Widerrufsrecht',
  'terms.section10.item1':
    '10.1. Privatkundinnen und Privatkunden haben ein gesetzliches Widerrufsrecht von 14 Tagen gemäß §312g BGB.',
  'terms.section10.item2':
    '10.2. Verlangt die Kundin bzw. der Kunde, dass die Beraterin innerhalb dieses Zeitraums mit der Leistung beginnt, wird anerkannt, dass das Widerrufsrecht insoweit erlischt, wie die Leistung bereits erbracht wurde.',
  'terms.section10.item3':
    '10.3. In den ersten 14 Tagen investiert die Beraterin ungefähr 60 % des gesamten Arbeitsaufwands des Plans (individueller schriftlicher Plan, erste Videogespräche und Schlafanalyse). Im Fall eines Widerrufs wird dieser anteilige Betrag berechnet und nur die Differenz für den noch nicht erbrachten Teil des Pakets erstattet.',
  'terms.section11.title': '11. Absage und Terminverschiebung',
  'terms.section11.item1':
    '11.1. Videotelefonate können bis 24 Stunden vorher verschoben werden.',
  'terms.section11.item2':
    '11.2. Erscheint die Kundin bzw. der Kunde ohne vorherige Mitteilung nicht zu einem Termin, gilt dieser als durchgeführt.',
  'terms.section11.item3':
    '11.3. Nach Beginn der Begleitung werden keine Rückerstattungen vorgenommen.',
  'terms.section12.title': '12. Schlussbestimmungen',
  'terms.section12.item1':
    '12.1. Sollte eine Bestimmung dieser AGB unwirksam sein, bleiben die übrigen Bestimmungen wirksam.',
  'terms.section12.item2':
    '12.2. Es gilt das Recht der Bundesrepublik Deutschland.',
  'terms.section12.item3':
    '12.3. Erfüllungsort und Gerichtsstand ist, soweit gesetzlich zulässig, der Wohnsitz der Beraterin.',
  'terms.service.section': 'Leistungsbedingungen',
  'terms.service.summary.title': 'Leistungsübersicht',
  'terms.service.summary.item1': 'Begleitung über 28 Kalendertage.',
  'terms.service.summary.item2':
    'Betreuungszeit zwischen 9:00 und 21:00 Uhr.',
  'terms.service.summary.item3':
    'Videogespräche, Nachrichten und Auswertung von Schlafprotokollen.',
  'terms.service.summary.item4':
    'Respektvoller, individueller und nicht medizinischer Ansatz.',
  'terms.keydata.title': 'Wichtige Daten',
  'terms.keydata.payment':
    'Die Zahlung ist vor Leistungsbeginn zu leisten, sofern nichts anderes ausdrücklich vereinbart wurde.',
  'terms.keydata.withdrawal':
    'Das gesetzliche Widerrufsrecht beträgt 14 Tage gemäß §312g BGB.',
  'terms.keydata.refunds':
    'Nach Beginn der Begleitung werden keine Rückerstattungen vorgenommen.',
  'terms.contact.title': 'Kontakt',
  'terms.contact.text': 'Bei Fragen zu diesen Bedingungen können Sie schreiben an',
  'terms.contact.email': 'info@bonanitsuenoinfantil.com',
  'terms.back': 'Zurück zur Startseite',

  // Privacy Policy
  'privacy.title': 'Datenschutzerklärung',
  'privacy.subtitle':
    'Informationen über die Verarbeitung personenbezogener Daten bei Bonanit.',
  'privacy.responsible': 'Verantwortliche Stelle',
  'privacy.validity': 'Stand',
  'privacy.legal.bases': 'Wesentliche Rechtsgrundlagen',
  'privacy.consent': 'Einwilligung',
  'privacy.contract': 'Vertragserfüllung und vorvertragliche Anfragen',
  'privacy.legal.obligation': 'Rechtliche Verpflichtung',
  'privacy.legitimate.interests': 'Berechtigte Interessen',
  'privacy.preamble': 'Präambel',
  'privacy.summary': 'Zusammenfassung der Verarbeitungen',
  'privacy.legal.bases.applicable': 'Anwendbare Rechtsgrundlagen',
  'privacy.security.measures': 'Sicherheitsmaßnahmen',
  'privacy.storage.general':
    'Allgemeine Hinweise zur Speicherung und Löschung von Daten',
  'privacy.rights': 'Rechte betroffener Personen',
  'privacy.commercial.services': 'Geschäftliche Leistungen',
  'privacy.online.offer': 'Bereitstellung des Onlineangebots und Webhosting',
  'privacy.cookies': 'Verwendung von Cookies',
  'privacy.contact.management': 'Kontakt- und Anfragenverwaltung',
  'privacy.messenger.communication': 'Kommunikation per Messenger',
  'privacy.web.analysis': 'Webanalyse, Monitoring und Optimierung',
  'privacy.online.marketing': 'Online-Marketing',
  'privacy.social.media': 'Präsenz in sozialen Netzwerken',
  'privacy.back': 'Zurück zur Startseite',
} as const;
